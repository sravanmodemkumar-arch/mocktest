...
**[Save as Draft]:** Creates `legal_contract` record with `status='DRAFT'`. Available to Contract Coordinator (#103) and Legal Officer (#75).
**[Send for Signature]:** Creates record with `status='SENT_FOR_SIGNATURE'` and dispatches e-signature request via DigiSign API (or email with PDF link if Manual Upload). POST to `/legal/contracts/send/`. Legal Officer (#75) must review before send for contracts with custom terms (flag: `requires_legal_review=true`).
